...
| 設定 UI（§7-2 の取得範囲の選択） | **実装済み**（ユーザースクリプトのパネル内。取得範囲＝全件 / 直近N日 / 指定日以降、システムメッセージ除外可否）。実機での確認は未実施 |
...
### 6.4 会話名（ファイル名・フロントマター用）

> **2026-08-13: ファイル名に会話名が入ることを実機で確認済み**
> （チャネル・チャット、コンソール貼り付け版・ユーザースクリプト版とも）。

会話名は DOM ではなく**画面（ブラウザのタブ）のタイトル**から取る。実測値（2026-08-13）:
...
- 先頭はアプリ内のセクション名（未読数 `(3) ` が付くことがある）、末尾はアプリ名。
  **どちらも UI 言語依存なので、文字列一致ではなく位置で落とす**（設定は `selectors.json` の `conversationTitle`）。
- チャネルは残り 2 つがチーム名・チャネル名、チャットは残り 1 つがチャット名。
- 会話名に区切り文字 ` | ` が含まれる場合は、最後のフィールドに区切りごと戻す。
  チーム名に含まれる場合だけは分離できない（実害が小さいので追わない）。
- **タイトルの更新は遅れることがある。** 実機で、収集開始時点では `'(3) Planner | Microsoft Teams'` と
  会話名を欠いた形になっており、収集後には入っている、という状況を観測した（2026-08-13）。
  そのため**開始時と終了時の両方を候補にして、名前が取れたほうを採用する**（`resolveConversationTitle()`）。
  両方で取れる場合は開始時のほうを使う（収集開始時点の会話を正とする）。
- 取り出せなければ埋めずに `conversation-title-unparsed`（info）を出し、ファイル名は従来の既定名になる。
  警告には試したタイトルをすべて残す（次の調査の手がかりにするため）。

ヘッダ要素から取る案もあったが、タブのタイトルで両方賄えるうえ DOM 構造の変化に強いのでこちらにした。
